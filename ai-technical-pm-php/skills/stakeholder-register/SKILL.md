...
All stakeholder data lives at `$VOOHY_HOME` (default: `~/voohy-reflections/`), **outside the public repo**.
...
If `~/voohy-reflections/` does not exist:
...
4. **Warn the user explicitly** before writing the first stakeholder file:
   > I'm about to create `~/voohy-reflections/`, which will hold candid notes about real people. This will live on your local machine only. Confirm the location, or set `$VOOHY_HOME` if you want a different one (e.g. an encrypted volume or a Dropbox folder).
...
Append to `~/voohy-reflections/stakeholders.json`:
...
- **Privacy is non-negotiable.** Re-warn if you notice the user's `$VOOHY_HOME` is inside a git repo that isn't already gitignoring it.
